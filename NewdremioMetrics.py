...
# Push Gateway URL
pgwendpoint = "http://localhost:9091"
...
sql_canary_query_elapsedms_value = 'dremio_sql_canary_query_elapsedms_Value'
...
def main():
	...
	if len(sys.argv) < 3:
		...
		if status == 0:
			...
			print('pgwendpoint :' ,pgwendpoint)
			print('pgw_api_url :',pgw_api_url)
			...
			for item in items:
				...
				if api_cluster_status_metric in item:
					print('api_cluster_status_metric : ' ,api_cluster_status_metric)
					...
					for record in item.items():
						if sql_executor_metric in record:
							...
							if jobName == dremioCluster:
								...
								push_sql_metric(sql_canary_query_elapsedms_value, dremioCluster, executorNode, 0)
			...
			print('mast coordinater is not down ')
			...
			# Canary elapsed time
			query = 'select count(*) from sys.memory'
			begin_ts = current_milli_time()
			cursor.execute(query)
			end_ts = current_milli_time()
			elapsed_ms = end_ts - begin_ts
			push_sql_metric(sql_canary_query_elapsedms_value, dremioCluster, 'dummy', elapsed_ms)
